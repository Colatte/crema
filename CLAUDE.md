...
│   ├── GITHUB.md                    # the settings that are not in the tree: branch protection on main AND dev (admins bound), the merge buttons, Dependabot's two targets — with the idempotent block that reapplies all of it
...
│   │   └── web/                     # what index.html actually serves, derived from the above by scripts/make-web-assets.sh — 5.5 MB of PNG becomes 171 KB of WebP, and the three HUD specimens are CROPPED, not merely resized (full frame they are ~90% empty wallpaper and the tile reads as a grey rectangle). Regenerate there, never hand-edit
├── design/
...
│   ├── check-catalog.py     # the String Catalog gate (seven rules + two "not verified" reports: multiline defaultValue and a call shape the checker does not read); CI and release.sh run it — Xcode enforces nothing
...
"Build & test" gates, administrators included (a direct push to `dev` or `main`
is rejected, not bypassed — docs/GITHUB.md); `dev` reaches `main` as a release —
or as a documentation-only merge (docs/DECISIONS.md:
main-takes-docs-outside-a-release). The branch, merge and issue rules live
there, not here.
...
<!-- docscheck: ignore C1 — the golden rule is the section right below; the checker keys the heading in Portuguese ("Regra de ouro") and this doc-set is English by decision, since Crema is a public GPL-3.0 repository whose contributors read it -->
...
- The exception that **stays detailed**: comments carrying expensive, durable knowledge — like the private-API rationale header in `Crema/Sources/Brightness/` (frameworks used, paths discarded and why, ownership/ID gotchas) — must still be objective, but the **content stays**; trimming decoration from these never means erasing the knowledge.
...
- **A reference to an internal doc is always self-sufficient.** The lesson lives **in the comment itself**; the ID (`docs/DECISIONS.md: J7-estado-do-outro-lado`) is a pointer for depth, never the only copy of the information — a reader without access to the doc still understands the why. Corollary: a round name lives only in PLAN.md's own round marker, where the ref beside it is what makes the name resolvable — code, DECISIONS.md and every other doc cite anchors, never round names, because outside the PLAN a round name is vocabulary with no referent.
...
- Presentation timers (e.g. the HUD revert) are cancellable `Task`s sleeping on the **`SleepClock`** protocol (an injectable clock in `Crema/Coordinator/`; production uses `ContinuousSleepClock`, tests use a fake clock and never actually sleep) — never `Timer`/RunLoop.
...
- **Layout of `Crema/Sources/`**: the protocols and the generic composite sources live at the **root**; each subdirectory is a technology with the concrete implementations. The interesting edge logic is extracted pure and testable — the Reconciler/Translation/Conversion pattern (`ScreenLockReconciler`, `AdapterPayloadTranslation`, `VolumeConversion`) — and the thin edge keeps only the system contact.
...
- **Catalog verbatim discipline**: `defaultValue` byte-for-byte identical to the `en` value, `extractionState` manual, a translated `pt-BR` unit, no orphan keys, the same specifiers in both languages, no glyph in any value. **`scripts/check-catalog.py` enforces it in CI — Xcode enforces nothing** (measured: with `extractionState: manual`, `xcstringstool sync` walks past an entry whose `en` diverged from the code without a warning), and its self-test runs first, because a checker that silently stops checking returns the same "clean".
...
- **No emoji in UI strings, none** — the glyph duplicates the sentence, VoiceOver reads it mid-sentence, and `✓` collides with NSMenu's own vocabulary, where a checkmark means a checked item (docs/DECISIONS.md: menu-status-before-warnings); `check-catalog.py` refuses any value carrying one (rule 7). The About signature is prose too: coffee is a word there, and the brand is written "Colatte" — the lowercase `colatte.` is the logo lettering, never typed in a text font (docs/INTERNATIONALIZATION.md).
...
- [ ] **Three grammar checks are suppressed, because the checker keys its structural positions in Portuguese and this doc-set is English** — `C1`/`F1`/`T3`, each suppressed at its site with the reason on the line; `T2` is paid for instead, with the field spelled `· módulo:` in PLAN.md. Reopening gate: the kit learning the English keywords.
...
None pending. No convention about how code is written in this repository is
waiting on a call; the one open decision in the system is the format of Lunar's
socket events, which gates T7.4 and lives in [PLAN.md](PLAN.md).
