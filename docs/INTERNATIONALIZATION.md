...
Every `String(localized:defaultValue:)` key exists in `Localizable.xcstrings` with a `defaultValue` byte-for-byte identical to the `en` value, `extractionState` manual, a semantic (not literal) key and a translated `pt-BR` unit — no orphan keys, with the same specifiers in both languages (a translation that drops or renames a `%@` is a broken format string served only to the Brazilian user), and no glyph in any value, on either side (the section below).
...
**The rule has no exception.** The About tab's signature once carried the app's only glyph ("made with ☕ by …"), exempted as brand rather than state; it now reads "made with coffee by Colatte", because the Colatte brand manual puts coffee in UI as a word, never an emoji, and because a plain sentence needs no separate VoiceOver label. With the exemption gone the rule is mechanical, and `check-catalog.py` enforces it as rule 7: any character of Unicode category So — every emoji, and ☕ ⚠ ✓ ⌘ with them — in any value of any language fails the gate, while the punctuation the catalog is full of (— … ·) is Po/Pd and passes. Catching ⌘ is intended: a shortcut is drawn by AppKit from the key equivalent, never typed into the title. The rule became enforceable only once it had no exception, because a checker cannot tell brand from state — a rule you live in violation of teaches you to ignore it.
