...
**The rule has no exception.** The About tab's signature once carried the app's only glyph ("made with ☕ by …"), exempted as brand rather than state; it now reads "made with coffee by Colatte", because the Colatte brand manual puts coffee in UI as a word, never an emoji, and because a plain sentence needs no separate VoiceOver label. The rule is mechanically verifiable and, with the exemption gone, the catalog contains no glyph at all — a rule you live in violation of teaches you to ignore it.
